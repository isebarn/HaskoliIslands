...
public class SQLHelper{
	...
	static String[] columnNames = new String[]{"id", "pickup", "destination", "phone"};
	static int[] rideData = new int[4];
	static LinkedList<int[]>  rides = new LinkedList(); //need a linked list becouse the result of a sqlite query doesnt have length with it
	static int[][] rideArray;
	static String selection = "select id, pickup, destination, phone from rides;";
	static String insertion = "insert into rides(pickup, destination, phone) values(?,?,?);";
	static String remove = "delete from rides where id is (?);";
	 ...
	 */
	public static void insertRide(int[] values){
		createConnection();
		try{
			connection.setAutoCommit(false);
	 ...
	 */
	public static void insertRide(int pickup, int dropoff, int phone){
		createConnection();
		try{
			connection.setAutoCommit(false);
			PreparedStatement statement = connection.prepareStatement(insertion);
	...
	public static void main(String[] args) {
		...
		hehe.insertRide(new int[]{0,4,5885522});
		hehe.insertRide(3,6,118);
		hehe.fetchRides();
		hehe.createRideArray();
		hehe.printMatrix(hehe.rideArray);
		System.out.println(" ");
		hehe.deleteRide(1);
		hehe.fetchRides();
		hehe.createRideArray();
