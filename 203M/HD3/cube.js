...
window.onload = function init()
{
    ...
    window.addEventListener("keydown", function(e){
        if (e.keyCode == up) scale -= 0.01
        if (e.keyCode == down) scale += 0.01
        if (e.keyCode == left) slide -= 0.01
        if (e.keyCode == right) slide += 0.01
...
function render()
{
    ...
    ctm = mult(translate(slide,0,0), ctm)
    ctm = mult(scalem(scale, scale, scale), ctm)
