...
var rVertices = [
    vec3(-0.5,-0.5,0.5),vec3(-0.5,0.5,0.5),vec3(0,0,1),
    vec3(-0.5,0.5,0.5),vec3(0.5,0.5,0.5),vec3(0,0,1),
    vec3(0.5,0.5,0.5),vec3(0.5,-0.5,0.5),vec3(0,0,1),
    vec3(0.5,-0.5,0.5),vec3(-0.5,-0.5,0.5),vec3(0,0,1),
    vec3(-0.5,-0.5,0.5),vec3(0.5,0.5,0.5),vec3(-0.5,0.5,0.5),
        ...
        switch( e.keyCode ) {
            case 38:    // up arrow
                if (view != 0) {
                    ...
                    x += 5 * Math.cos(radians(theta))
                    y += 5 * Math.sin(radians(theta))
                }
                break;
            case 40:    // down arrow
                if (view != 0) {
            ...
            case 48:    // 1: distant and stationary viewpoint
                view = 0;
                document.getElementById("Viewpoint").innerHTML = "0: Fjarlægt sjónarhorn";
                break;            
            ...
            case 57:    // 8: from beside the car
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Á húsi";
                break;                
...
function render()
{
    ...
    case 0:
        ...
        mv = lookAt( vec3(x+turn, y, 5), vec3(10000*Math.cos(radians(theta)), 10000*Math.sin(radians(theta)), 0.0), vec3(0.0, 0.0, 1.0) );
        drawScenery( mv );
    ...
    case 1:
            // Distant and stationary viewpoint
	    mv = lookAt( vec3(250.0+turn, 0.0, 100.0+height), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0) );
	    drawScenery( mv );
	    mv = mult( mv, translate( carXPos, carYPos, 0.0 ) );
	    mv = mult( mv, rotateZ( carDirection ) ) ;
	    drawCar( mv );
	...
	case 2:
	    ...
	    mv = lookAt( vec3(75.0+turn, 0.0, 5.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
	    drawScenery( mv );
	    mv = mult( mv, translate(carXPos, carYPos, 0.0) );
	    mv = mult( mv, rotateZ( carDirection ) ) ;
	    drawCar( mv );
	...
	case 3:
	    ...
	    mv = lookAt( vec3(125.0+turn, 0.0, 5.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
	    drawScenery( mv );
	    mv = mult( mv, translate(carXPos, carYPos, 0.0) );
	    mv = mult( mv, rotateZ( carDirection ) ) ;
	    drawCar( mv );
    ...
    case 9:
        // Static viewpoint inside the track; camera follows car
        mv = lookAt( vec3(-20+turn, 50, 6.0), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
        drawScenery( mv );
        mv = mult( mv, translate(carXPos, carYPos, 0.0) );
        mv = mult( mv, rotateZ( carDirection ) ) ;
        drawCar( mv );
