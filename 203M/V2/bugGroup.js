function bugGroup(sizeOfGroup){
	...
	this.alignment = function(){
		for (var i = this.bugs.length - 1; i >= 0; i--) {
			...
			for (var j = 0; j < this.bugs.length; j++) {
				if (i != j && length(subtract(this.bugs[i].position, this.bugs[j].position)) < 0.2) {
	...
	this.cohesion = function(){
		for (var i = this.bugs.length - 1; i >= 0; i--) {
			...
			for (var j = 0; j < this.bugs.length; j++) {
				if (i != j && length(subtract(this.bugs[i].position, this.bugs[j].position)) < 0.2 && length(subtract(this.bugs[i].position, this.bugs[j].position)) > 0.1) {
